...
public class BatchExtractMaterials : EditorWindow
{
    ...
    private class RemapAllPopup : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if ((ev.type == UnityEngine.EventType.DragPerform || ev.type == UnityEngine.EventType.DragUpdated) && GUILayoutUtility.GetLastRect().Contains(ev.mousePosition))
            {
                DragAndDrop.visualMode = DragAndDropVisualMode.Copy;
                if (ev.type == UnityEngine.EventType.DragPerform)
                {
    ...
    private string materialsFolder = "Assets/Materials/CityEngineMaterials";
    private string fbxFolder = string.Empty;
    private List<ExtractData> modelData = new List<ExtractData>(16);

#if UNITY_2019_1_OR_NEWER
    private bool remappedMaterialNamesMustMatch = false;
    private bool remappedMaterialPropertiesMustMatch = false;
    private bool remappedTextureNamesMustMatch = true;
    private bool dontRemapExtractedMaterials = false;
    ...
    //[MenuItem("Buildings/Batch Extract Materials")]
    ...
    private void DrawModelsToProcessList()
    {
        ...
        fbxFolder = EditorGUILayout.TextField("Import FBX From", fbxFolder);
        ...
        if (GUILayout.Button("o", GL_WIDTH_25))
        {
            string selectedPath = EditorUtility.OpenFolderPanel("Choose fbx directory", "Assets", "");
            if (!string.IsNullOrEmpty(selectedPath))
            {
                selectedPath = selectedPath.Replace('\\', '/') + "/";
                ...
                if (relativePathIndex > 0)
                {
                    fbxFolder = selectedPath.Substring(relativePathIndex, selectedPath.Length - relativePathIndex - 1);
                }
            }
            Debug.Log(fbxFolder);
            ...
            var paths = Directory.GetFiles(appPath + fbxFolder, "*.fbx", SearchOption.TopDirectoryOnly);
            modelData.Clear();
            foreach (var p in paths)
            {
                var o = AssetDatabase.LoadAssetAtPath<GameObject>(p.Replace(appPath, ""));
    ...
    private void DrawMaterialRemapConditionsField()
    {
        ...
        remappedTextureNamesMustMatch = EditorGUILayout.ToggleLeft("Texture names must match", remappedTextureNamesMustMatch);
                ...
                for (int materialIndex = 0; materialIndex < materials.arraySize; materialIndex++)
                {
                    ...
                    else
                    {
                        ...
                        if (!materialAlreadyExtracted || !dontRemapExtractedMaterials)
                        {
                            ...
                            else if (remappedTextureNamesMustMatch)
                            {
                                remappedMaterial = GetMaterialWithTexture(duplicateMaterialsLookup, material.mainTexture);
                            }
                        }
    ...
    private Material GetMaterialWithTexture(Dictionary<int, HashSet<Material>> lookup, Texture texture)
    {
        foreach (HashSet<Material> allMaterials in lookup.Values)
        {
            foreach (Material material in allMaterials)
            {
                ...
                /*
                else if (material.mainTexture.name == texture.name)
                {
                    return material;
                }
                */
